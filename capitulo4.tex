...
\newcommand{\AprovacaoHipotese}[3]{Ao analisar os
resultados obtidos para as respostas
dos participantes para as afirmativas de avaliação
da hipótese, identificamos que a favorabilidade
para #1as afirmativas #2atendem os critérios
previamente definidos em #3 replicações.}

\newcommand{\AprovacaoHipoteseResultado}[4]{
O resultado {\ifthenelse{\equal{#1}{0}}{}{não }}permite concluir
...
\newcommand{\HipoteseFavorabilidade}[4]{Para a afirmativa
``#1'' na replicação do problema ``#2'' no
semestre #3 a percepção de favorabilidade,
nos critérios definidos, ficou em $#4\%$.
}

\newcommand{\HipoteseFavorabilidadeDestaque}[4]{O
principal destaque para esta avaliação está na afirmativa
``#1'' na replicação do problema ``#2'' no
